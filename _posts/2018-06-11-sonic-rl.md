...
**Avoid Overfitting**. To prevent the joint agent from overfitting to the training levels, I sampled several joint trained agents at various points of joint training and performed a cross validation analysis on the local validation levels. Below is the chart showing the training curve (on the local validation set) of several PPO models initialized by different joint agents sampled at various points during joint training. The sampled points are 20M, 60M, 100M, 130M, 170M, and 210M total timesteps. The training curve for PPO with no pretraining (bottom orange) is also included for comparison.

![Local Validation Joint Model](/img/sonic_chart2.png){:width="600"}<br />

First thing we notice is that **PPO agents initialized with pretrained models work a lot better than the PPO agent with no pretraining (bottom orange)**. Second, we observe that the differences in local validation scores of the PPO agents initialized with joint models sampled at different timesteps are well within the score variance, which is around +/- 1000. Hence, I simply picked the model from around 50 million steps to be the joint agent for initializing the PPO agent for the leaderboard test levels.
